...
export async function POST(req: NextRequest) {
  try {
    ...
    const { title, date, description, image, order } = body;
    if (!title || !date || !description || !image || order === undefined) {
      return NextResponse.json({ error: "Missing required fields." }, { status: 400 });
    }

    const newItem = await Timeline.create({ title, date, description, image, order });
...
export async function PUT(req: NextRequest) {
  try {
    ...
    const body = await req.json();
    const updatedItem = await Timeline.findByIdAndUpdate(id, body, { new: true });
    if (!updatedItem) {
